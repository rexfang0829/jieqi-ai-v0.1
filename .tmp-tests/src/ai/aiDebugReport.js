...
exports.formatAiDebugReport = formatAiDebugReport;
const moveNotation_1 = require("../game/moveNotation");
const pieceText_1 = require("../game/pieceText");
function bool(v) {
...
function fmtTrace(t) {
    const lines = [
        '  棋步：' + (0, moveNotation_1.moveText)(t.move),
        '  score：' + t.score,
        '  reason：' + t.reason,
...
function boardSnapshot(state) {
    ...
    for (let r = 0; r < 10; r++) {
        const cells = state.board[r].map(piece => {
            ...
            if (piece.revealed) {
                return side + (0, pieceText_1.pieceTypeName)(piece.side, piece.realType);
            }
            return side + '暗' + (0, pieceText_1.pieceTypeName)(piece.side, piece.originalType);
        });
...
function formatAiDebugReport(input) {
    ...
    if (recent.length > 0) {
        ...
        for (let i = 0; i < recent.length; i++) {
            lines.push((offset + i + 1) + '. ' + (0, moveNotation_1.moveText)(recent[i]));
        }
    }
    /* AI recommendation */
    lines.push('');
    lines.push('--- AI 建議 ---');
    if (r.move) {
        lines.push('推薦棋步：' + (0, moveNotation_1.moveText)(r.move));
    ...
    /* Selected trace */
    const selectedTrace = r.traces?.find(t => r.move &&
    ...
    if (traces && traces.length > 0) {
        ...
        for (let i = 0; i < top5.length; i++) {
            const t = top5[i];
            lines.push((i + 1) + '. ' + (0, moveNotation_1.moveText)(t.move) + '｜' + t.score + '｜' + t.reason);
        }
    }
    lines.push('');
    lines.push('==================');
